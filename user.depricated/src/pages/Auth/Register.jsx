...
class Register extends React.Component {
  ...
  handleSubmit = (values, setSubmitting) => {
    ...
    axios({
      method: 'POST',
      url: `${process.env.serviceAPI}/auth/register`,
      data,
    })
      .then((res) => {
        if (res && res.data && res.data.Result && res.data.Result.Message && res.data.Result.Message === 'register successfully') {
          axios({
            method: 'POST',
            url: `${process.env.serviceAPI}/auth/login`,
            data,
          })
            .then((loginRes) => {
              if (loginRes.data && loginRes.data.Result && loginRes.data.Result.Token) {
                Cookies.set('auth', loginRes.data.Result.Token, { domain: '.constant.money', expires: 30 });
                    ...
                    ({
                      ...
                      <form onSubmit={handleSubmit}>
                        ...
                        <div className="c-field c-submit">
                          <button className="c-btn c-btn-primary c-block" type="submit">
