...
const showMessage = (msg, type = "warning", time = 2) => {
  if (type === "success") {
    message.success(<MessageContent>{msg}</MessageContent>, time);
  } else if (type === "error") {
    message.error(<MessageContent>{msg}</MessageContent>, time);
  } else {
    message.warning(<MessageContent>{msg}</MessageContent>, time);
  }
};
...
export default class extends Component {
  ...
  render() {
    ...
    const btnText =
      side == "buy" ? (
        <IntlMessages id="Exchange.OrderForm.PlaceOrderBuy" />
      ) : (
        <IntlMessages id="Exchange.OrderForm.PlaceOrderSell" />
      );
        ...
        <ContentHolder>
          ...
          >
            <RadioButton
              value="buy"
              style={
                this.state.side == "buy" ? OrderSide.Buy : OrderSide.Default
              }
            >
              BUY
            </RadioButton>
            <RadioButton
              value="sell"
              style={
                this.state.side == "sell" ? OrderSide.Sell : OrderSide.Default
              }
            >
              SELL
            </RadioButton>
        ...
        <ContentHolder style={{ marginTop: "1rem" }}>
          ...
          >
            <TabPane tab="Market" key="1">
              <Label>
                <IntlMessages id="Exchange.OrderForm.Amount" />
              </Label>
              <InputGroup>
                <Input
            ...
            <TabPane tab="Limit" key="2">
              ...
              <Label>
                <IntlMessages id="Exchange.OrderForm.Price" />
              </Label>
              <InputGroup>
                <Input
